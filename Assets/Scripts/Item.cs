...
public class Item : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        player_pickup = GetComponent<PickUp>();
    }
    ...
    void OnCollisionEnter( Collision collision )
    {
        ...
        {
            print("from item: collided with player!");
        }
        else
        {
    ...
    void OnTriggerEnter( Collider collision)
    {
        if(collision.gameObject.tag == "Player")
        {
            print("from TRIGGERENTER of object, player entered");
            // display text if the player is looking at the GameObject

            // 
            player_pickup.addToInventory(gameObject, name);
        }
    ...
    {
        print("triggerEXIT --> player exited");
        // disable the text
    }
